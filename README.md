...
这个示例 Vault 也可以直接用 Aino LifeOS 打开。仓库内置了 `.lifeos/settings.json`，Aino 会自动识别周期笔记、PARA 目录、任务标题和模板路径。

请保留 `.lifeos/` 和 `.obsidian/` 两个隐藏目录：前者是 Aino 的 Vault 配置，后者是 Obsidian 兼容配置。
